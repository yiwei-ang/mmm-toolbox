...
def create_stan_function_block():
    ...
    """
    return function_block_code


def create_stan_data_block(config_df: pd.DataFrame):
    spaces = "    "
...
def create_stan_parameters_block(config_df: pd.DataFrame):
    spaces = "    "
...
def create_stan_transformed_parameters_block(config_df):
    spaces = "    "
    tp_block_code = "transformed parameters {\n"
    tp_block_code += f"{spaces}vector[N] mu = rep_vector(0, N);\n"
    tp_block_code += f"{spaces}for (n in 1:N) {{\n"
    ...
    for _, row in config_df.iterrows():
        ...
        if category == 'MEDIA':
            ...
            tp_block_code += f"{spaces}{spaces}mu[n] += hill_function({variable}[n], {beta_name}, {A_name}, {B_name});\n"

        if category == 'BASE':
            tp_block_code += f"{spaces}{spaces}mu[n] += {beta_name} * {variable}[n];\n"

    # Add the revenue likelihood
    tp_block_code += f"{spaces}}}\n"
    tp_block_code += "}\n"

    return tp_block_code


def create_stan_model_block(config_df: pd.DataFrame):
    spaces = "    "
    kpi_name = config_df[config_df['Category'] == 'KPI']['Variable'].tolist()[0]
    model_code = "model {\n"
    ...
    for _, row in config_df.iterrows():
        ...
        if category == 'MEDIA':
            ...
            model_code += f"{spaces}{beta_name} ~ normal({row['Beta_Init']}, {row['Prior']});\n"
            model_code += f"{spaces}{A_name} ~ normal({row['Parameter_A_Init']}, {row['Prior']});\n"
            model_code += f"{spaces}{B_name} ~ normal({row['Parameter_B_Init']}, {row['Prior']});\n"

        if category == 'BASE':
            model_code += f"{spaces}{variable} ~ normal({row['Beta_Init']}, {config_df[config_df['Variable'] == variable]['Prior'].values[0]});\n"
    ...
    model_code += f"{spaces}{spaces}{kpi_name}[n] ~ normal(mu[n], sigma);\n"
    model_code += f"{spaces}}}\n"
...
def create_stan_generated_quantities_block(config_df):
    spaces = "    "
    kpi_name = config_df[config_df['Category'] == 'KPI']['Variable'].tolist()[0]
    gq_block_code = "generated quantities {\n"

    # Define the mu vector
    gq_block_code += f"{spaces}vector[N] log_lik;\n"

    # Calculate mu using the same logic as in the model block
    gq_block_code += f"{spaces}for (n in 1:N) {{\n"
    gq_block_code += f"{spaces}{spaces}log_lik[n]=normal_lpdf({kpi_name}[n] | mu[n], sigma);\n"

    gq_block_code += f"{spaces}}}\n"
    gq_block_code += "}\n"

    return gq_block_code
